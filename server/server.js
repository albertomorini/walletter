...
https.createServer(options,(req,res)=>{
    ...
    req.on("end",()=>{
        ...
        if (req.url=="/getAuth"){ //a user's trying to authenticate it self
            mongoExecutor.getUser(bodyDict.Email,bodyDict.Password).then(resQuery=>{
                if(resQuery==null){
            ...
            mongoExecutor.insertUser({ "email": bodyDict.Email, "psw": bodyDict.Password, "premium": false }).then(resQuery=>{
                sendResponse(res,200,{"usr":resQuery})
            }).catch(err=>{
        ...
        if(req.url=="/getExport"){
            mongoExecutor.getAllTransaction(bodyDict.Email, bodyDict.Password).then(resQuery =>{
                if(resQuery!=null){
        ...
        if(req.url=="/transaction"){ // update or insert new a transaction
            ...
            mongoExecutor.deleteTransaction(bodyDict.idTransaction,bodyDict.Email,bodyDict.Password).then(resQuery=>{
                if(resQuery.acknowledged){
        ...
        if (req.url =="/getExistingReferences"){
            mongoExecutor.getExistingReferences(bodyDict.Email,bodyDict.Password).then(resQuery=>{
                if(resQuery==null){
